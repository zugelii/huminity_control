...
void setup() 
{
  ...
  digitalWrite(MY_CE_PIN, LOW);      // chip enable
  digitalWrite(MY_CS_PIN, HIGH);                 // Spi disable  
//  RX_Mode();                        // set RX mode
//   TX_Mode();                       // set TX mode
}
#define REV
	...
	for(;;)
	{
     ...
     SPI_RW_Reg(WRITE_REG+STATUS,status);                             // clear RX_DR or TX_DS or MAX_RT interrupt flag
		if(status&RX_DR)                                                 // if receive data ready (TX_DS) interrupt
		{
			...
			SPI_RW_Reg(FLUSH_RX,0xff);                                        // clear RX_FIFO
 			for(int i=0; i<32; i++)
 			{
			...
			if (rx_buf[10]==4)
			{
				digitalWrite(MY_LED_BUILTIN,!digitalRead(MY_LED_BUILTIN));
			}
...
void RX_Mode(void)
{
  ...
  delay_loc(10);
  digitalWrite(MY_CE_PIN, HIGH);                             // Set MY_CE_PIN pin high to enable RX device
